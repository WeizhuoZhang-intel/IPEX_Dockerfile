                ...
                for weight in data['modelargs'][mode]['weighttype']:
                    if weight == "int8":
                        if model_id == "EleutherAI/gpt-neox-20b":
                            ...
                            lines.append(f"python run_falcon_quantization.py --ipex-weight-only-quantization --output-dir {data['modelargs'][mode]['outputdir']} --int8-bf16-mixed --weight-dtype INT8 --lowp-mode BF16 -m <MODEL_ID> --config-file /root/workspace/IPEX_Dockerfile/tiiuae_falcon-40b_config.json")
                        else:
                                ...
                                for beam in data['modelargs'][mode]['greedy']:
                                    ...
                                    if beam == False:
                                        lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --dtype {dtype} --input-tokens {input_token} \
                                                    --max-new-tokens {output_token} --ipex --deployment-mode --token-latency --num-iter 50 --prompt One more question, where did this docker image come from? Since this GNR cannot connect to intel's network, it should not be able to be pulled down. Did you copy it from another machine? 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_{beam}_NUMA_{numa}_BF16.log") 
                                    elif beam == True:
                                        lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark --greedy -m {data['modelargs'][mode]['shardpath']} --dtype {dtype} --input-tokens {input_token} \
                                                    --max-new-tokens {output_token} --ipex --deployment-mode --token-latency --num-iter 50 --prompt One more question, where did this docker image come from? Since this GNR cannot connect to intels network, it should not be able to be pulled down. Did you copy it from another machine? 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_{beam}_NUMA_{numa}_BF16.log") 
