...
def generate_commands(yml_file,mode,extra_kmp):
    ...
    with open(generated_file, "w") as runfile:
        ...
        if mode.endswith('smacc'):
            ...
            lines.append(f"rm -rf {data['modelargs'][mode]['outputdir']}")
            lines.append(f"mkdir -p {data['modelargs'][mode]['outputdir']}")
            lines.append(f"python run_llama_quantization.py --ipex_smooth_quant --lambada --output_dir {data['modelargs'][mode]['outputdir']} --jit --int8_bf16_mixed -m meta-llama/Llama-2-7b-hf --int8-qconfig /home/mint/shard/config/qconfig.json")
