...
def generate_commands(yml_file,mode,extra_kmp):
    ...
    with open(generated_file, "w") as runfile:
                    ...
                    for input_token in data['modelargs'][mode]['inputtokens']:
                        for output_token in data['modelargs'][mode]['maxnewtokens']:
                            ...
                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} \
                                         --benchmark -m {model_id} --input-tokens {input_token} --max-new-tokens {output_token}  --num-iter 50 --dtype {dtype} --ipex --deployment-mode --token-latency --token-latency  \
                ...
                for dtype in data['modelargs'][mode]['dtype']:
                    for input_token in data['modelargs'][mode]['inputtokens']:
                        for output_token in data['modelargs'][mode]['maxnewtokens']:
                            ...
                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} \
                                         --benchmark -m {model_id} --input-tokens {input_token} --max-new-tokens {output_token}  --num-iter 50 --dtype {dtype} --ipex --torch-compile --token-latency --token-latency  \
                ...
                for dtype in data['modelargs'][mode]['dtype']:
                    for input_token in data['modelargs'][mode]['inputtokens']:
                        for output_token in data['modelargs'][mode]['maxnewtokens']:
                            lines.append(f"nohup bash /root/workspace/get_mem.sh >> $log_dir/mem-usage-llm_default_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_False_NUMA_1_BF16.log 2>&1 || true &")
                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} \
                                         --benchmark -m {model_id} --input-tokens {input_token} --max-new-tokens {output_token}  --num-iter 50 --dtype {dtype} --prompt 'It is done, and submitted. You can play Survival of the Tastiest on the Android, and on the web. Playing on the web works, but you have to simulate multiple touch for table moving and that can be a bit confusing. There is a lot I would like to talk about. I will go through every topic, insted of making the typical what went right/wrong list. Concept Working over the theme was probably one of the hardest tasks which I had to face. Originally, I had an idea of what kind of game I wanted to develop, gameplay wise - something with a lot of enemies/actors' --token-latency --token-latency  --deployment-mode \
                                            2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_False_NUMA_1_BF16.log")
                            lines.append(f"collect_perf_logs_llm llm_default_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_False_NUMA_1_BF16.log")
                                        ...
                                        if model_id == "EleutherAI/gpt-neox-20b":
                                            ...
                                            if beam == False:
                                                ...
                                                lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                            --input-tokens {input_token} --max-new-tokens {output_token} --int8 --weight-dtype INT8 --lowp-mode BF16 -m {model_id} --benchmark --token-latency --token-latency  --num-iter 50 \
                                                            2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT8.log")
                                            elif beam == True:
                                                lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                            --input-tokens {input_token} --max-new-tokens {output_token} --int8 --weight-dtype INT8 --lowp-mode BF16 -m {model_id} --greedy --benchmark --token-latency --token-latency  --num-iter 50 \
                                                            2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT8.log")                                                
                                            ...
                                            if beam == False:
                                                lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                            --input-tokens {input_token} --max-new-tokens {output_token} --int8-bf16-mixed --weight-dtype INT8 --lowp-mode BF16 -m {model_id} --benchmark --token-latency --token-latency  --num-iter 50 \
                                                            2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT8.log")
                                            elif beam == True:
                                                lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                            --input-tokens {input_token} --max-new-tokens {output_token} --int8-bf16-mixed --weight-dtype INT8 --lowp-mode BF16 -m {model_id} --greedy --benchmark --token-latency --token-latency  --num-iter 50 \
                                                            2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT8.log")                                                
                                            lines.append(f"collect_perf_logs_llm llm_default_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT8.log")
                                ...
                                for beam in data['modelargs'][mode]['greedy']:
                                    if model_id == "EleutherAI/gpt-neox-20b":
                                        ...
                                        if beam == False:
                                            ...
                                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                        --input-tokens {input_token} --max-new-tokens {output_token} --int8 --weight-dtype INT4 --lowp-mode INT8 -m {model_id} --benchmark --token-latency --token-latency  --num-iter 50 \
                                                        2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")
                                        elif beam == True:
                                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                        --input-tokens {input_token} --max-new-tokens {output_token} --int8 --weight-dtype INT4 --lowp-mode INT8 -m {model_id} --greedy --benchmark --token-latency --token-latency  --num-iter 50 \
                                                        2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")
                                        ...
                                        if beam == False:
                                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                        --input-tokens {input_token} --max-new-tokens {output_token} --int8-bf16-mixed --weight-dtype INT4 --lowp-mode INT8 -m {model_id} --benchmark --token-latency --token-latency  --num-iter 50 \
                                                        2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")
                                        elif beam == True:
                                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                        --input-tokens {input_token} --max-new-tokens {output_token} --int8-bf16-mixed --weight-dtype INT4 --lowp-mode INT8 -m {model_id} --greedy --benchmark --token-latency --token-latency  --num-iter 50 \
                                                        2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")
                                        lines.append(f"collect_perf_logs_llm llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")
                    ...
                    for output_token in data['modelargs'][mode]['maxnewtokens']:
                        for beam in data['modelargs'][mode]['greedy']:
                            if model_id == "EleutherAI/gpt-neox-20b":
                                ...
                                if beam == False:
                                    lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                --input-tokens {input_token} --max-new-tokens {output_token} --int8 -m {model_id} --benchmark --token-latency --token-latency  --num-iter 50 \
                                                2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_BF16.log")
                                elif beam == True:
                                    lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                --input-tokens {input_token} --max-new-tokens {output_token} --int8 -m {model_id} --greedy --benchmark --token-latency --token-latency  --num-iter 50 \
                                                2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_BF16.log") 
                                ...
                                if beam == False:
                                    lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                --input-tokens {input_token} --max-new-tokens {output_token} --int8-bf16-mixed -m {model_id} --benchmark --token-latency --token-latency  --num-iter 50 \
                                                2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_BF16.log")
                                elif beam == True:
                                    lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                --input-tokens {input_token} --max-new-tokens {output_token} --int8-bf16-mixed -m {model_id} --greedy --benchmark --token-latency --token-latency  --num-iter 50 \
                                                2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_int8_{input_token}-{output_token}_greedy_{beam}_NUMA_1_BF16.log")                                    
                                ...
                                for beam in data['modelargs'][mode]['greedy']:
                                    ...
                                    if beam == False:
                                        lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --dtype {dtype} --input-tokens {input_token} \
                                                    --max-new-tokens {output_token} --ipex --deployment-mode --token-latency --token-latency  --num-iter 50 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_{beam}_NUMA_{numa}_BF16.log") 
                                    elif beam == True:
                        ...
                        for output_token in data['modelargs'][mode]['maxnewtokens']:
                            for numa in data['modelargs'][mode]['localrank']:
                                ...
                                lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --device {data['modelargs'][mode]['device'][0]} --benchmark -m {model_id} --dtype {dtype} --input-tokens {input_token} \
                                            --max-new-tokens {output_token} --ipex --jit --token-latency --token-latency  --num-iter 50 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_True_NUMA_{numa}_BF16.log") 
                                    ...
                                    if weight == "int8":
                                        if model_id == "EleutherAI/gpt-neox-20b":
                                            ...
                                            lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --weight-dtype INT8 --lowp-mode BF16 --dtype float32 --input-tokens {input_token} \
                                                        --max-new-tokens {output_token} --ipex --deployment-mode --ipex-weight-only-quantization --token-latency --token-latency  --num-iter 50 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_False_NUMA_{numa}_INT8.log") 
                                        ...
                                        if model_id == "EleutherAI/gpt-neox-20b":
                                            ...
                                            lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --weight-dtype INT4 --lowp-mode INT8 --dtype float32 --input-tokens {input_token} \
                                                        --max-new-tokens {output_token} --ipex --deployment-mode --ipex-weight-only-quantization --token-latency --token-latency  --num-iter 50 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_False_NUMA_{numa}_INT4.log") 
                    ...
                    for output_token in data['modelargs'][mode]['maxnewtokens']:
                        for beam in data['modelargs'][mode]['greedy']:
                            ...
                            if beam == False:
                                ...
                                if model_id == "EleutherAI/gpt-neox-20b":
                                    lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                --input-tokens {input_token} --max-new-tokens {output_token} --int8 -m {model_id} --benchmark --token-latency --token-latency  --greedy --num-iter 50 \
                                                2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")
                                    lines.append(f"collect_perf_logs_llm llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")  
                                else:
                                    lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} \
                                                --input-tokens {input_token} --max-new-tokens {output_token} --int8-bf16-mixed -m {model_id} --benchmark --token-latency --token-latency  --greedy --num-iter 50 \
                                                2>&1 | tee -a $log_dir/llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")   
                                    lines.append(f"collect_perf_logs_llm llm_default_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_{beam}_NUMA_1_INT4.log")                                                                              
        ...
        # if mode.endswith('gptqint4'):
            ...
            #     lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptnamerun']} -m {model_id} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} --benchmark --int8-bf16-mixed --token-latency --token-latency ")
        ...
        #                         --ipex-tpp --ipex --jit --token-latency --token-latency  2>&1 | tee -a $log_dir/llm_{mode}_{model_id.replace('/','-')}_{dtype}_{input_token}_greedy_{beam}.log")
        #                     else:
        #                         lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --device {data['modelargs'][mode]['device'][0]} -m {model_id} --input-tokens {input_token} --dtype {dtype} --ipex-tpp --ipex --jit --token-latency --token-latency  2>&1 | tee -a $log_dir/llm_{mode}_{model_id.replace('/','-')}_{dtype}_{input_token}_greedy_{beam}.log")
        #                     lines.append(f"collect_perf_logs_llm llm_{mode}_{model_id.replace('/','-')}_{dtype}_{input_token}_greedy_{beam}.log")
        ...
        #                 for beam in data['modelargs'][mode]['greedy']:
        #                     lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --device cpu --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} --input-tokens {input_token} --max-new-tokens {output} --jit --int8-bf16-mixed -m {model_id} --lambada --benchmark --token-latency --token-latency  \
        ...
        #             for output in data['modelargs'][mode]['maxnewtokens']:
        #                 for beam in data['modelargs'][mode]['greedy']:
        #                     lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python {data['modelargs'][mode]['scriptname']} --device cpu --input-tokens {input_token} --max-new-tokens {output} --jit -m {model_id} --dtype bfloat16 --lambada --benchmark --token-latency --token-latency  \
        ...
        #                         --ipex --jit --token-latency --token-latency  --token-latency  2>&1 | tee -a $log_dir/llm_{mode}_{model_id.replace('/','-')}_{dtype}_{input_token}_greedy_{beam}.log") 
        #                     else:
