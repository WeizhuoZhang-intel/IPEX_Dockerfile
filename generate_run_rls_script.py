                ...
                for dtype in data['modelargs'][mode]['dtype']:
                    for input_token in data['modelargs'][mode]['inputtokens']:
                        for output_token in data['modelargs'][mode]['maxnewtokens']:
                            ...
                            lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python run.py \
                                         --benchmark -m {model_id} --input-tokens {input_token} --max-new-tokens {output_token}  --num-iter 50 --int8-bf16-mixed --ipex-smooth-quant --output-dir {data['modelargs'][mode]['outputdir']} --deployment-mode --token-latency   \
                        ...
                        for output_token in data['modelargs'][mode]['maxnewtokens']:
                            for numa in data['modelargs'][mode]['localrank']:
                                ...
                                lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list run.py --benchmark -m {model_id} --dtype {dtype} --input-tokens {input_token} \
                                            --max-new-tokens {output_token} --output-dir {data['modelargs'][mode]['outputdir']} --ipex --deployment-mode --token-latency --num-iter 50 --autotp 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_{dtype}_{input_token}-{output_token}_greedy_False_NUMA_{numa}_BF16.log") 
                            ...
                            for numa in data['modelargs'][mode]['localrank']:
                                ...
                                if 'neox' in model_id:
                                    lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list run.py --benchmark -m {model_id} --int8 --input-tokens {input_token} \
                                                --max-new-tokens {output_token} --ipex --ipex-weight-only-quantization --profile --output-dir {data['modelargs'][mode]['outputdir']} --deployment-mode --token-latency --num-iter 20 --autotp 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_False_NUMA_{numa}_BF16.log") 
                                elif 'falcon' in model_id:
                                    lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list run.py --benchmark -m {model_id} --int8-bf16-mixed --input-tokens {input_token} \
                                                --max-new-tokens {output_token} --ipex --ipex-weight-only-quantization --profile --output-dir {data['modelargs'][mode]['outputdir']} --deployment-mode --token-latency --num-iter 20 --autotp --config-file /root/workspace/IPEX_Dockerfile/utils/model_config/tiiuae_falcon-40b_config.json 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_False_NUMA_{numa}_BF16.log") 
                                else:
                                    lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list run.py --benchmark -m {model_id} --int8-bf16-mixed --input-tokens {input_token} \
                                                --max-new-tokens {output_token} --ipex --ipex-weight-only-quantization --profile --output-dir {data['modelargs'][mode]['outputdir']} --deployment-mode --token-latency --num-iter 20 --autotp 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_False_NUMA_{numa}_BF16.log") 
                                    ...
                                    if weight == "int8":
                                        if model_id == "EleutherAI/gpt-neox-20b":
                                            ...
                                            lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --weight-dtype INT8 --lowp-mode BF16 --dtype float32 --input-tokens {input_token} \
                                                        --max-new-tokens {output_token} --ipex --deployment-mode --ipex-weight-only-quantization --token-latency --profile --num-iter 20 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_False_NUMA_{numa}_INT8.log") 
                                        ...
                                        if model_id == "EleutherAI/gpt-neox-20b":
                                            ...
                                            lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --weight-dtype INT4 --lowp-mode INT8 --dtype float32 --input-tokens {input_token} \
                                                        --max-new-tokens {output_token} --ipex --deployment-mode --ipex-weight-only-quantization --token-latency --profile --num-iter 20 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_False_NUMA_{numa}_INT4.log") 
