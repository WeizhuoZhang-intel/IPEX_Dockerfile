...
def generate_commands(yml_file,mode,extra_kmp):
    ...
    with open(generated_file, "w") as runfile:
                                    ...
                                    if weight == "int8":
                                        if model_id == "EleutherAI/gpt-neox-20b":
                                            ...
                                            lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --weight-dtype INT8 --lowp-mode BF16 --dtype float32 --input-tokens {input_token} \
                                                        --max-new-tokens {output_token} --ipex --deployment-mode --ipex-weight-only-quantization --token-latency --profile --num-iter 50 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int8_{input_token}-{output_token}_greedy_False_NUMA_{numa}_INT8.log") 
                                        ...
                                        if model_id == "EleutherAI/gpt-neox-20b":
                                            ...
                                            lines.append(f"deepspeed --bind_cores_to_rank --num_accelerators {numa} --bind_core_list $core_list {data['modelargs'][mode]['scriptname']} --benchmark -m {data['modelargs'][mode]['shardpath']} --weight-dtype INT4 --lowp-mode INT8 --dtype float32 --input-tokens {input_token} \
                                                        --max-new-tokens {output_token} --ipex --deployment-mode --ipex-weight-only-quantization --token-latency --profile --num-iter 50 2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woq-int4_{input_token}-{output_token}_greedy_False_NUMA_{numa}_INT4.log") 
        ...
        #                         --ipex --jit --token-latency --profile --token-latency --profile  2>&1 | tee -a $log_dir/llm_{mode}_{model_id.replace('/','-')}_{dtype}_{input_token}_greedy_{beam}.log") 
        #                     else:
