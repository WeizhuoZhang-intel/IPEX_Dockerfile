                        ...
                        for output_token in data['modelargs'][mode]['maxnewtokens']:
                            for beam in data['modelargs'][mode]['greedy']:
                                if beam == True:
                                    ...
                                    lines.append(f"OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']} numactl -N {data['launcher']['numactlN']} -m {data['launcher']['numactlM']} python run.py \
                                                --benchmark -m {model_id} --input-tokens {input_token} --max-new-tokens {output_token} --greedy  --num-iter 50 --int8-bf16-mixed --ipex-smooth-quant --deployment-mode --output-dir {data['modelargs'][mode]['outputdir']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} --dataset NeelNanda/pile-10k --alpha {data['modelargs'][mode]['alpha']} --token-latency   \
                                ...
                                for beam in data['modelargs'][mode]['greedy']:
                                    for bs in data['modelargs'][mode]['batchsize']:
                                        if beam == True:
                                            ...
                                            lines.append(f"deepspeed --bind_cores_to_rank --master_port 29500 --bind_core_list 0-63 run.py --batch-size {bs} --benchmark -m {model_id} --dtype {dtype} --input-tokens {input_token} \
                            ...
                            for numa in data['modelargs'][mode]['localrank']:
                                ...
                                if 'neox' in model_id:
                                    ...
                                    for beam in data['modelargs'][mode]['greedy']:
                                        for bs in data['modelargs'][mode]['batchsize']:
                                            if beam == True:
                                                lines.append(f"deepspeed --bind_cores_to_rank --master_port 29500 --bind_core_list 0-63 run.py --benchmark -m {model_id} --int8-bf16-mixed --input-tokens {input_token} \
