                ...
                for dtype in data['modelargs'][mode]['dtype']:
                    ...
                    if model_id == "EleutherAI/gpt-neox-20b":
                        ...
                        if 'config' in dtype:
                            ...
                            lines.append(f"python {data['modelargs'][mode]['scriptname']} --ipex-smooth-quant --output-dir {data['modelargs'][mode]['outdir']} --model-id {model_id} --int8-bf16-mixed --qconfig-summary-file /data1/qmodel/rlsipexbf16/best_configure.json")
                            lines.append(f"export OMP_NUM_THREADS={data['launcher']['OMP_NUM_THREADS']}")
