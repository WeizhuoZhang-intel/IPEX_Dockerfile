                    ...
                    for input_token in data['modelargs'][mode]['inputtokens']:
                        for beam in data['modelargs'][mode]['greedy']:
                            for numa in data['modelargs'][mode]['localrank']:
                                ...
                                if beam == True:
                                    lines.append(f"deepspeed --enable_each_rank_log=/root/workspace/IPEX_Dockerfile/log_dir/rank0/,/root/workspace/IPEX_Dockerfile/log_dir/rank1/  --bind_cores_to_rank --num_accelerators {numa} --bind_core_list 0-63 {data['modelargs'][mode]['scriptname']} --benchmark --device {data['modelargs'][mode]['device'][0]} -m {model_id} --greedy --dtype {dtype} --input-tokens {input_token} \
