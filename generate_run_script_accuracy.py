        ...
        if mode == 'deepspeed':
            ...
            for model_id in data['modelargs'][mode]['modelid']:
                for dtype in data['modelargs'][mode]['dtype']:
                    ...
                    lines.append(f"mkdir {data['modelargs'][mode]['outdir']}")
                    lines.append(f"deepspeed --bind_cores_to_rank run_generation_with_deepspeed.py -m {data['modelargs'][mode]['shardpath']} --prompt mint --output-dir {data['modelargs'][mode]['outdir']} --dtype float32 --ipex --jit --ipex-weight-only-quantization")
