                                    ...
                                    for rank in data['modelargs'][mode]['localrank']:
                                        ...
                                        if rank == 2:
                                            ...
                                            if beam == True:   
                                                ...
                                                lines.append(f"mpiexec.hydra -l -np 2 \"python single_instance/run_generation.py -m {model_id} --deployment-mode  --benchmark --num-iter {data['launcher']['iternum']} --token-latency --num-warmup 10 --greedy --batch-size {bs} --input-tokens {input_token} --max-new-tokens {output_token} --ipex --dtype bfloat16\" 2>&1 | tee -a $log_dir/llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log")
                                            else:   
                                                ...
                                                #                 2>&1 | tee -a $log_dir/llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log")                                            
                                                lines.append(f"mpiexec.hydra -l -np 2 \"python single_instance/run_generation.py -m {model_id} --deployment-mode  --benchmark --num-iter {data['launcher']['iternum']} --token-latency --num-warmup 10 --batch-size {bs} --input-tokens {input_token} --max-new-tokens {output_token} --ipex --dtype bfloat16\" 2>&1 | tee -a $log_dir/llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log")
                                            ...
                                            if beam == True:   
                                                # lines.append(f"RUN_WORKLOAD='python single_instance/run_generation.py -m {model_id} --deployment-mode  --benchmark --num-iter {data['launcher']['iternum']} --token-latency --num-warmup 10 --batch-size {bs} --greedy --input-tokens {input_token} --max-new-tokens {output_token} --ipex --dtype bfloat16'   \
                                                #                 2>&1 | tee -a $log_dir/llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log")
                                                lines.append(f"mpiexec.hydra -l -np 4 \"python single_instance/run_generation.py -m {model_id} --deployment-mode --greedy  --benchmark --num-iter {data['launcher']['iternum']} --token-latency --num-warmup 10 --batch-size {bs} --input-tokens {input_token} --max-new-tokens {output_token} --ipex --dtype bfloat16\" 2>&1 | tee -a $log_dir/llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log ")
                                            
                                            else:   
                                                # lines.append(f"RUN_WORKLOAD='python single_instance/run_generation.py -m {model_id} --deployment-mode  --benchmark --num-iter {data['launcher']['iternum']} --token-latency --num-warmup 10 --batch-size {bs} --input-tokens {input_token} --max-new-tokens {output_token} --ipex --dtype bfloat16' \
                                                #                 2>&1 | tee -a $log_dir/llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log")                                            
                                                lines.append(f"mpiexec.hydra -l -np 4 \"python single_instance/run_generation.py -m {model_id} --deployment-mode  --benchmark --num-iter {data['launcher']['iternum']} --token-latency --num-warmup 10 --batch-size {bs} --input-tokens {input_token} --max-new-tokens {output_token} --ipex --dtype bfloat16\" 2>&1 | tee -a $log_dir/llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log ")
                                            lines.append(f"collect_perf_logs_llm llm_tensor_{(model_id.replace('/','-')).replace('_','-')}_{dtype}_{input_token}-{output_token}-{bs}_greedy_{beam}_NUMA_{rank}_{data['launcher']['hw']}.log")
