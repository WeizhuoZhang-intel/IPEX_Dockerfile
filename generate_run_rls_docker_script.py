                                ...
                                for bs in data['modelargs'][mode]['batchsize']:
                                    for rank in data['modelargs'][mode]['localrank']:
                                        ...
                                        if rank == 2:
                                            ...
                                            if beam == True:   
                                                lines.append(f"RUN_WORKLOAD='python single_instance/run_generation.py -m {model_id} --deployment-mode  --benchmark --num-iter {data['launcher']['iternum']} --num-warmup {data['launcher']['warmup']} --token-latency --batch-size {bs} --greedy --input-tokens {input_token} --max-new-tokens {output_token} --ipex --dtype bfloat16'")
