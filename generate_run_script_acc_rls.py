...
parser.add_argument("--acc_rls",action="store_true",default=False,help="only for accuracy release regular track")
            ...
            for model_id in data['modelargs'][mode]['modelid']:
                for dtype in data['modelargs'][mode]['dtype']:
                    if dtype == "bfloat16" and model_id != "EleutherAI/gpt-neox-20b":      
                        lines.append(f"deepspeed  --num_gpus 2 --master_addr `hostname -I | sed -e 's/\s.*$//'` --bind_cores_to_rank {data['modelargs'][mode]['scriptname']} --model {model_id} --dtype bfloat16 --ipex --jit --tasks lambada_openai --accuracy-only \
                                    2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_{dtype}_accuracy.log")
                    elif dtype == "int8":
                        if model_id == "EleutherAI/gpt-neox-20b":
                            lines.append(f"deepspeed  --num_gpus 2 --master_addr `hostname -I | sed -e 's/\s.*$//'` --bind_cores_to_rank {data['modelargs'][mode]['scriptname']} --model {model_id} --ipex-weight-only-quantization --dtype float32 --ipex --jit --tasks lambada_openai --accuracy-only \
                                        2>&1 | tee -a $log_dir/llm_deepspeed_{model_id.replace('/','-')}_woqint8_accuracy.log")
                        else:
                            lines.append(f"deepspeed  --num_gpus 2 --master_addr `hostname -I | sed -e 's/\s.*$//'` --bind_cores_to_rank {data['modelargs'][mode]['scriptname']} --model {model_id} --ipex-weight-only-quantization --int8-bf16-mixed --ipex --jit --tasks lambada_openai --accuracy-only \
...
if __name__ == '__main__':
    ...
    if args.acc_rls:
        yml_file = 'bench_release_accuracy.yml'
    data = yaml.load(open(yml_file, 'r'),Loader=yaml.FullLoader) 
