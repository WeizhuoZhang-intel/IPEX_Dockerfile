    ...
    with open(generated_file, "w") as runfile:
        ...
        if mode.endswith('int8'):
            for model_id in data['modelargs'][mode]['modelid']:
                ...
                for input_token in data['modelargs'][mode]['inputtokens']:
                    for beam in data['modelargs'][mode]['greedy']:
                        lines.append(f"python {data['modelargs'][mode]['scriptname']} --quantized-model-path {data['modelargs'][mode]['quantizedmodelpath']} --input-tokens {input_token} --jit --int8-bf16-mixed -m {model_id} --lambada --benchmark --token-latency")
